	...
	$.ready.then(function(){
		...
		siteIntrvl = setInterval(function () {
			...
			if(isPageLoaded) {
				...
				$('head').append( $('<link rel="stylesheet" type="text/css"/>').attr('href', '/css/inners_' + pageLang + '.css') );
				...
				$.cachedScript('/jsm/vendor.js').done(function(script, textStatus) {
					...
					if(!isMobile) {
						...
						curTL1.to('.cur i', 0.5, { scale: 2, autoAlpha: 1, ease: Back.easeInOut }, 0)
						...
						$('._curTL1').mouseenter(function(){
							...
							if(title) { $('.cur strong').text(title) }
						...
						$('a, ._curTL2, .tab.flex, .filters_toggle, .equalizer, .circle_set, .audio_circle, ._link, .filter_head, .letters span, .option, .select_set, .form_btn, .circle_mag, .filters_close, .circle_btn, .font_circ:not(.disabled)').mouseenter(function(){

							if(!$(this).hasClass('active') && !$(this).hasClass('_curTL1')) {
...
function fire(){
	...
	if(pageHead.length != 0) {
		...
		if(pageLang == 'en') {
			...
			headSplit1 = new SplitText(pageHead, {type:"chars", charsClass:"SplitClass"});
			headSplit1Target = headSplit1.chars;
		} else {

			headSplit1 = new SplitText(pageHead, {type:"words", wordsClass:"SplitClass"});
			headSplit1Target = headSplit1.words;
		}

		TweenMax.staggerFrom(headSplit1Target, 1, { x: pageLang == 'en' ? 100 : -100, autoAlpha: 0, ease:Power3.easeOut, delay: 2 }, 0.05, function(){
	...
	if(page == 'editions') { buildEditions(); };
	...
	if(aniUP.length != 0) {
		...
		$.each(aniUP, function(i,element) {
			...
			let tl = new TimelineMax({paused:true}),
				$this = $(this),
				split = new SplitText($this, {type:"words", wordsClass:"SplitClass"}),
				words = split.words;

			tl.staggerTo(words, 0.3, { y: -10, autoAlpha: 0, ease:Power3.easeIn }, 0.05)

			.set(words, { y: 10})

			.staggerTo(words, 0.3, { y: 0, autoAlpha: 1, ease:Power3.easeOut }, 0.05)
...
function buildEditions(){
	...
	$('.tab_section').each(function(i) {
		...
		if(wrapImgs.length != 0) {
			...
			$thisTab.find('.fluid_img').wrapAll('<div class="fluid_imgs flex"></div>')

		}

		if ( i % 2 == 0) {

			$thisTab.find('.fluid_img').eq(1).add($thisTab.find('.fluid_img').eq(2)).wrapAll($('<div/>',{ class: 'fluid_imgs_row flex'}))
			$thisTab.find('.fluid_img').eq(4).add($thisTab.find('.fluid_img').eq(5)).wrapAll($('<div/>',{ class: 'fluid_imgs_row flex'}))

		} else {

			$thisTab.find('.fluid_img').eq(0).add($thisTab.find('.fluid_img').eq(1)).wrapAll($('<div/>',{ class: 'fluid_imgs_row flex'}))
			$thisTab.find('.fluid_img').eq(4).add($thisTab.find('.fluid_img').eq(5)).wrapAll($('<div/>',{ class: 'fluid_imgs_row flex'}))
...
function pageScroll(val){
	...
	if(Sketch && Sketch.materials) {
		...
		Sketch.imageStore.forEach(o=>{
			...
			if (o.mesh.material.uniforms.uMouseMoveStrength.value > -1 && lastScrollVal != scrollVal && $.inArray(o, isPlaying) == -1) {
				...
				tl.to(o.mesh.material.uniforms.uMouseMoveStrength, 1, {value: -1, ease: Power3.easeOut })

				.to(o.mesh.material.uniforms.uMouseMoveStrength, 1, { value: 0, ease: Back.easeInOut })
	...
	item.each(function(){
		...
		getItemY >= 1 ? $this.addClass('full-width') : $this.removeClass('full-width');
	...
	$('.tab_section').each(function(i) {
		...
		if(typeof val.currentElements[getScrollID] === 'object') {
			...
			let progress = ((height - $(val.currentElements[getScrollID].el).offset().top) * 100) / $(val.currentElements[getScrollID].el).innerHeight();
	...
	if(pageProg.length != 0) {
		...
		TweenMax.set(pageProg, {scaleX: ((height + scrollVal) / $('main').innerHeight())})
	...
	if(sign.length != 0) {
		...
		if(sign.isInViewport() && !sign.hasClass('ready')) {
			...
			TweenMax.staggerFromTo(signStroke, 1, {drawSVG:"0%"}, {drawSVG:"100%", ease: Power3.easeInOut}, 0.2)
	...
	if(signs.length != 0) {
		...
		signs.each(function(){
			...
			if($this.isInViewport() && !$this.hasClass('ready')) {
				...
				TweenMax.staggerFromTo(signsStroke, 1, {drawSVG:"0%"}, {drawSVG:"100%", ease: Power3.easeInOut}, 0.2)
	...
	if($('.line_shape').length !=0 && scrollVal > 10) {
		...
		$('.line_shape').each(function(){
			...
			if($this.isInViewport() && !$this.hasClass('ready')) {
				...
				TweenMax.fromTo($this.find('path'), 4, {drawSVG:"0%"}, {drawSVG:"100%", ease: Power3.easeOut})
...
function showForm(target) {
	...
	.staggerFrom(target.find('._ele'), 0.5, {y: 20, autoAlpha: 0, ease: Power3.easeOut}, 0.2, 0)

	.staggerFromTo(target.find('.input_border line'), 0.5, {drawSVG:"0%"}, {drawSVG:"100%", ease: Power3.easeOut}, 0.05, 0)

	.fromTo(target.find('.inline_submit i'), 0.5, {scaleY: 0, transformOrigin: '100% 100%'}, {scaleY: 1, ease: Power3.easeOut}, 0.5)

	.staggerFromTo(target.find('.input_set label, .inline_submit span'), 1, {y: 20, autoAlpha: 0}, {y: 0, autoAlpha: 1, ease: Power3.easeOut}, 0.05, 0.5)
...
function tagsAdj() {
	...
	if(tagSelector.length != 0 ) {
		...
		if($('.has_background.filters').length == 0) {
			...
			if(tagSelector.find('a:not([data-href='+id+'])').hasClass('active')) {
			...
			if(isTagsFlickity) {
				if(width > 640) {
					if(!tagSelector.find('a.active:last').closest('li').hasClass('is-selected')) {
						tagsCarousel.flickity( 'select', tagSelector.find('a.active:last').closest('li').index());
					}
				} else {
					tagsCarousel.flickity( 'select', tagSelector.find('a.active:last').closest('li').index() , false, true);
				}
			}
		...
		if(val == 1) {
			...
			if ( isTagsFlickity == false ) {
				...
				if(width > 960) {
					...
					$('#sections').css('padding-top', getTagsH - tagSelector.innerHeight())
				...
				if(!isMenu && scroll) {scroll.update();};
...
function changeTitle(id) {
	...
	$.data(this, 'ed_Timer', setTimeout(function() {
		...
		if(pageLang == 'en') {
			...
			split = new SplitText(getTitle, {type:"chars", charsClass:"SplitClass"});

			headSplit = split.chars;

		} else {

			split = new SplitText(getTitle, {type:"words", wordsClass:"SplitClass"});

			headSplit = split.words;

		}

		edTL.staggerTo(headSplit, 0.8, {x: -90, autoAlpha: 0, ease: Power3.easeIn}, 0.025, 0)
		...
		.call(function(){
			...
			edTL.staggerFrom(headSplit, 0.8, {x: 90, autoAlpha: 0, ease: Power3.easeOut}, 0.025, 0)
...
function changeEdition(id) {
	...
	if(page == 'editions') {
		...
		if (getTags) { 
			...
			if (getTagsColors) { tagsColors.push(getTagsColors.split(',')) }

			if (getTagsURLS) { tagsURLS.push(getTagsURLS.split(',')) }
			...
			tags[0].forEach(function(i, o){
				...
				$('.tags').append('<a href="'+tagsURLS[0][o]+'" style="color:'+tagsColors[0][o]+';">'+i+'</a>')
		...
		$.data(this, 'cd_Timer', setTimeout(function() {
			...
			cdTL.staggerTo(headSplit, 0.8, {x: -90, autoAlpha: 0, ease: Power3.easeIn}, 0.025, 0)
			...
			.call(function(){
				...
				cdTL.staggerFrom(headSplit, 0.8, {x: 90, autoAlpha: 0, ease: Power3.easeOut}, 0.025, 0)
	...
	if(tabsCarousel) {
		...
		if(!isForceTab) {
			if(width > 640) {
				...
				tabsCarousel.flickity( 'select', $('.tab.active:last').index() , false, true);
			}
...
function globalFunc(){
	...
	if($('.audio_circle').length != 0) {
		...
		$('.audio_circle').on('click', function(){
			...
			if(!$this.hasClass('read')) {
				...
				responsiveVoice.speak(document.getElementsByClassName(selector)[0].textContent, pageLang == 'en' ? "UK English Male" : "Arabic Male", {onstart: function(){ $this.add('.'+ selector).addClass('playing') }, onend: function(){ $this.add('.'+ selector).removeClass('read playing')} });

			} else {

				$('.audio_circle').add('.'+ selector).removeClass('read playing')
	...
	$('#increase').parent('.font_circ').click(function(){
	...
	$('#decrease').parent('.font_circ').click(function(){
	...
	if(circle.length != 0) {
		...
		holdTL.fromTo(circleWrap, 1, {scale: 1}, {scale: 1.1, ease: Power3.easeInOut}, 0)

		.fromTo(circleBorder, 1, {drawSVG:"0%"}, {drawSVG:"100%", ease: Power3.easeInOut}, 0)
	...
	if(teamSet.length != 0) {
		...
		teamCircle.each(function(i,element){
			...
			if(border.length != 0 ) {
				...
				tl.fromTo(border, 1, {drawSVG:"0%"}, {drawSVG:"100%", ease: Power3.easeInOut}, 0)

				.staggerFrom(circSplit.words, 1, { y: 20, autoAlpha: 0, ease:Power3.easeInOut }, 0.1, 0)
		...
		teamCircle.on('mousemove touchstart', function(){
		...
		tabs.click(function(){
			...
			if(!isDragging) {
				...
				let id = $(this).attr('data-id'),
					sectionOffset = $('section[data-scroll-id='+id+']').offset().top + scrollVal,
					before = tabsWrap.innerHeight();

				if($('.has_background.filters').length != 0) {
					before = tagSelector.closest('.sub-menu').innerHeight() - parseInt(tagSelector.closest('.sub-menu').css('padding-top')) + tabsWrap.innerHeight();
				}
			...
			if(!isDragging) {
				...
				if(isTagsFlickity) {
					if(width > 640) {
						...
						tagsCarousel.flickity( 'select', $this.closest('li').index(), false, true);
					}
				}
		...
		galleryCarousel = gallerySet.flickity({
			...
			on: {
				ready: function() {
					...
					TweenMax.staggerFrom('.is-selected .gallery_block', 1, {autoAlpha: 0, x: pageLang == 'en' ? 300 : -300, ease: Power3.easeOut, delay: 1.5}, 0.05, function(){
						if(!isMobile) { galleryInt(); }
			...
			$(document).on('mousemove', function(e){
				...
				galleryBlock.each(function(i){
					...
					let $this = $(this),
						...
						dist = Math.floor(Math.sqrt(Math.pow(getX - (offsetLeft+($this.width()/2)), 2) + Math.pow(getY - (offsetTop+($this.height()/2)), 2)));
						zIndexVal = scale(Math.abs(Math.floor( Math.pow((getX) - (offsetLeft + ($this.width()/2)), 1))), 0, Math.abs(Math.floor( Math.pow((offsetLeft + ($this.width()/2)), 1) )), 3, 0).toFixed(2)
						scaleVal = scale(dist, 0, 300, 3, 0).toFixed(2)

					TweenMax.to($this, 1, { zIndex: limitNumberWithinRange(zIndexVal * 100, 0, 999), ease: Power3.easeOut})
					TweenMax.to($this.find('.gallery_img'), 1, { z: limitNumberWithinRange(scaleVal, 0, 3), ease: Power3.easeOut})
	...
	function onWindowResize() {
		...
		$.data(this, 'resizeTimer', setTimeout(function() {
			...
			if(isFired){$('body').removeClass('progress');}
	...
	if($('.list').length != 0) {
		...
		listTL.staggerTo('.list i', 0.3, {y: -20, autoAlpha: 0, ease: Power3.easeIn}, 0.03)

		.set('.list i', {y: 20})

		.staggerTo('.list i', 0.3, {y: 0, autoAlpha: 1, ease: Power3.easeOut}, 0.03)
	...
	if($('.filters_wrap').length != 0) {
		...
		filtersTL.set('.filters_wrap', {autoAlpha: 1}, 0)

		.to('.mode_set, footer', 0.5, {autoAlpha: 0, ease: Power3.easeInOut}, 0)

		.from('.filters_set', 0.5, {x: pageLang == 'en' ? '100%' : '-100%', ease: Power3.easeInOut}, 0)

		.from('.filters_wrap > i', 0.5, {autoAlpha: 0, ease: Power3.easeInOut}, 0)

		.staggerFrom('.filters_set ._ele', 0.5, {y: 50, autoAlpha: 0, ease: Power3.easeOut}, 0.05, 0.5)
		...
		fCircleTL.staggerTo('.filters_circle i', 0.3, { y: -10, autoAlpha: 0, ease:Power3.easeIn }, 0.05)

		.set('.filters_circle i', { y: 10})

		.staggerTo('.filters_circle i', 0.3, { y: 0, autoAlpha: 1, ease:Power3.easeOut }, 0.05)
		...
		$(document).on('click', '.option', function(e){
			...
			if(!$this.hasClass('active')) {
				...
				id == 'writer' ? writerArr.splice( $.inArray(val, writerArr), 1 ) : catArr.splice( $.inArray(val, catArr), 1 );
	...
	if(!isMobile) {
		...
		$.each(menuLinks, function(i,element) {
			...
			if(pageLang == 'ar') {
				...
				Split = new SplitText($this, {type:"chars", charsClass:"SplitClass"});
				Chars = Split.chars;

			}

			tl.staggerTo(Chars, 0.3, { y: -5, autoAlpha: 0, ease:Power3.easeIn }, 0.01)

			.set(Chars, { y: 5})

			.staggerTo(Chars, 0.3, { y: 0, autoAlpha: 1, ease:Power3.easeOut }, 0.01)
	...
	tagSelector.find('li a').click(function(){
		...
		if(page) {
			...
			TweenMax.to('#getContent', {autoAlpha: 0, ease: Power3.easeOut, onComplete: function(){
				...
				$.ajax({
					...
					success: function(data) {
						...
						function init(){
							...
							TweenMax.to('#getContent', {autoAlpha: 1, ease: Power3.easeOut})
	...
	$('.mstones_content ul li').click(function(){
		...
		if(!$(this).hasClass('active') && canAnimateMS) {
			...
			var headSplit,
				headTarget,
				titleSplit = new SplitText(titleWrap, {type:"words", wordsClass:"SplitClass"}),
				paraSplit = new SplitText(textWrap, {type:"lines", linesClass:"SplitClass"});

				if(pageLang == 'en') {
					headSplit = new SplitText(h1, {type:"chars", charsClass:"SplitClass"});
					headTarget = headSplit.chars;
				} else {
					headSplit = new SplitText(h1, {type:"words", wordsClass:"SplitClass"});
			...
			tl.staggerFromTo('.mstones_images.active .mstones_img', 0.5, {y: 0, autoAlpha: 1}, {y: -50, autoAlpha: 0, ease: Power3.easeIn}, 0.05, 0)

			.set(headTarget, {transformOrigin: "center center "+tOval+"px"}, 0)

			.staggerTo(headTarget, 0.5, {rotationX: 90, autoAlpha: 0, ease: Power3.easeIn}, 0.05, 0.1)

			.staggerTo(titleSplit.words, 0.5, {y: -20, autoAlpha: 0, ease: Power3.easeIn}, 0.01, 0)

			.staggerTo(paraSplit.lines, 0.5, {y: -20, autoAlpha: 0, ease: Power3.easeIn}, 0.05, 0)
			...
			.call(function(){
				...
				tl.set(headTarget, {transformOrigin: "center center "+tOval+"px"}, 0)

				.staggerFrom(titleSplit.words, 1, {y: 20, autoAlpha: 0, ease: Power3.easeOut}, 0.05, 0)

				.staggerFrom(headTarget, 1, {rotationX: -90, autoAlpha: 0, ease: Power3.easeOut}, 0.05, 0.1)

				.staggerFrom(paraSplit.lines, 1, {y: 20, autoAlpha: 0, ease: Power3.easeOut}, 0.1, 0.5)

				.staggerFromTo(newM.find('.mstones_img'), 1, {y: 100, autoAlpha: 0}, {y: 0, autoAlpha: 1, ease: Power3.easeOut}, 0.05, 0)
...
function appendImgs(){
	...
	appendIMGs.each(function(i){
		...
		$('<img src="'+ s +'">').on('load',function(){
			...
			$.data(this, 'imgsTimer', setTimeout(function() {
	...
	appendBGs.each(function(i){
		...
		t.removeAttr('data-src').css({ 'background-image': 'url('+ s +')' }).removeClass('load_bg');

	});

	if($('.table_set.has_cards').length != 0) {tablesJS();}

	if($('.row_logo_set').length != 0) {

		var isHover = true;

		$('.table_set:not(.has_cards)').mouseenter(function(){
		...
		$(document).on('mousemove touchstart touchmove', function(e){
			...
			if(plus.length != 0 && isHover) {
				...
				plus.each(function(){
					...
					let plusBounds = {w: $this.innerWidth()/2, h: $this.innerHeight()/2, x: $this.position().left, y: $this.position().top},
						getMouse = {x: getX - wrap.offset().left, y: getY - wrap.offset().top};

					TweenMax.to($this.find('.row_logo_an'), 0.3, {x: getMouse.x - plusBounds.w - plusBounds.x, y: getMouse.y - plusBounds.h - plusBounds.y})
	...
	if($('#scratch').length != 0) {
		...
		function handlePercentage(filledInPixels) {
			...
			if (filledInPixels >= 80) {
				isDrawing = false;
				TweenMax.to('.canvas', 0.5, {autoAlpha: 0, ease: Power3.easeOut, onComplete: function(){
		...
		function handleMouseDown(e) {
			...
			if($('.scratch_tip').length != 0) {
				TweenMax.to('.scratch_tip', 1, {autoAlpha: 0, ease: Power3.easeOut, onComplete: function(){
					$('.scratch_tip').remove();
				}})
			}
		}
...
function tablesJS(){
	...
	if(!isMobile) {
		...
		$('.table_row').each(function(i){
			...
			$('.image').eq(i).find('i').css({'background-image': 'url('+img+')' })
		...
		$('.table_row').mouseenter(function(){
			...
			$('.image').eq(id).find('i').css({'background-image': 'url('+oldIMG+')' })
		...
		$('.table_set').mousemove(function(){
			...
			if(!isLeaveing) {
				...
				TweenMax.to(cardIMG, 0.6, {autoAlpha: 0, ease: Power3.easeOut, delay: 0.2})
			}
		...
		$('.table_row').each(function(i){
			...
			$(this).find('.table_col:first-child').prepend('<div class="col_image"><i class="full_bg"></i></div>')

			$(this).find('.col_image').find('i').css({'background-image': 'url('+img+')' })
...
function animateCard(){
	...
	$('.table_row.selected').length != 0 ? ofY = $('.table_row.selected').offset().top : ofY = 0;

	let xPos = curX - (cardWidth/2),
		yPos = curY - (cardHeight/2) + scrollVal,
		imgX = (curX - $('.table_set').offset().left - (width/2) + (cardWidth/2)) * -0.2,
	...
	if(firstTime) {
		...
		TweenMax.to(cardIMGActive.find('i'), 0.3, {x: imgX, y: imgY, ease: Power3.easeOut, onComplete: function(){
	...
	TweenMax.staggerTo(cardIMGNotActive, 0.5, {x: xPos, y: yPos, ease: Power3.easeOut, delay: function(index, target){
	...
	if( $('#container').length == 0 ) {
		...
		Sketch = {
			...
			init: function (options) {
				...
				if(!isSafari){this.renderer.setPixelRatio(window.devicePixelRatio)}
				this.container.appendChild( this.renderer.domElement );
				this.images = [...document.querySelectorAll('.preloadIMG')];

				const preloadImages = new Promise((resolve, reject) => {
					imagesLoaded(document.querySelectorAll('.preloadIMG'), { background: true }, resolve)
			...
			addImages: function () { 
				...
				this.imageStore = this.images.map(img=>{
					...
					material.uniforms.uResolution.value.x = window.devicePixelRatio * bounds.width;
					material.uniforms.uResolution.value.y = window.devicePixelRatio * bounds.height;
					...
					this.imageStore.forEach(o=>{
						...
						if($('.list').length != 0) {
							if(o.img.parentNode.classList.contains('active')) {
								TweenMax.to(o.mesh.material.uniforms.percent, 1, {value: 1, ease: Power3.easeOut })
							} else {
								TweenMax.to(o.mesh.material.uniforms.percent, 1, {value: 0.3, ease: Power3.easeOut })
							}
						}
						if(o.img.parentNode.classList.contains('cover_image')) {
							TweenMax.to(o.mesh.material.uniforms.percent, 1, {value: 0.8, ease: Power3.easeOut })
						}
					})
